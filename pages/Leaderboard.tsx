    ...
    <div className="animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-3 gap-4 mb-10 items-end">
        {[leaders[1], leaders[0], leaders[2]].map((leader, i) => (
          <div key={leader.rank} className={`flex flex-col items-center ${i === 1 ? 'pb-8' : ''}`}>
            <div className="relative mb-3">
              <div className={`rounded-full border-4 overflow-hidden ${i === 1 ? 'w-24 h-24 border-zinc-100' : 'w-16 h-16 border-zinc-800'}`}>
            ...
            <h4 className={`font-bold text-center ${i === 1 ? 'text-lg' : 'text-sm'} truncate w-full`}>{leader.name}</h4>
            <p className="text-[10px] text-zinc-600 font-bold uppercase">{leader.karma.toLocaleString()} K</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl">
        ...
        {leaders.slice(3).map((leader, i) => (
          <div 
            key={leader.rank} 
            className={`grid grid-cols-12 gap-4 px-6 py-5 items-center hover:bg-zinc-800/30 transition-colors ${i !== leaders.slice(3).length - 1 ? 'border-b border-zinc-800/50' : ''}`}
          >
            <div className="col-span-1 font-bold text-zinc-500 text-sm">#{leader.rank}</div>
            <div className="col-span-6 flex items-center gap-3">
              <img src={leader.avatar} className="w-8 h-8 rounded-full border border-zinc-800" alt={leader.name} />
              <div>
            ...
            <div className="col-span-5 text-right">
              <div className="text-sm font-bold text-zinc-100">{leader.karma.toLocaleString()}</div>
      ...
      <div className="mt-8 p-6 glass rounded-3xl flex items-center justify-between border border-white/10">
        <div className="flex items-center gap-4">
           ...
           <div>
             ...
             <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">Next rank: Mentor • +1,200 Karma</p>
           </div>
        </div>
